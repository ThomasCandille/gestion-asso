...
import { isTerminalStatus } from "./event-rules";
import {
  eventFiltersSchema,
  eventFormSchema,
  type EventFilters,
...
function assertCanManageEventType(actor: AppSession, eventType: EventType) {
  ...
  if (actor.role === "POLE_LEAD") {
    ...
    if (!allowed) {
      throw new EventPermissionError(
        "Vous n'avez pas acces a ce type d'evenement.",
      );
    }
  }
}
...
export async function createEvent(actor: AppSession, input: unknown) {
  ...
  const event = await prisma.event.create({
    data: {
      ...
      startsAt: normalizeOptionalDate(parsed.startsAt),
      endsAt: normalizeOptionalDate(parsed.endsAt),
